 ...
 * 
 * Measures timing for specific orderbook operations on CPU:
 * 1. ADD (non-matching inserts)
 * 2. LIMIT order insert + match
 * 3. CANCEL operations
 * 4. MARKET order insert + match
 * 
 * Parallel to GPU benchmark for direct comparison
 */
...
/**
 * Process messages through a single CPU orderbook
 */
void process_messages_single(OrderbookCPU& book, const Message* messages, int num_messages) {
...
/**
 * Process messages across batch of orderbooks sequentially
 */
void process_messages_batch(OrderbookBatchCPU& batch, const Message* messages, 
...
ScenarioResults run_scenario(
    ...
    if (!cpu_batch.allocate(scenario.num_books, scenario.n_orders, scenario.n_trades)) {
    ...
    if (!scenario.time_setup && !scenario.setup_messages.empty()) {
        ...
        process_messages_batch(cpu_batch, scenario.setup_messages.data(), setup_msgs_per_book);
    }
    ...
    // Warm-up run (for cache warming)
    std::cout << "  Warm-up run..." << std::endl;
    process_messages_batch(cpu_batch, messages_to_time.data(), test_msgs_per_book);
...
void print_comparison(const std::vector<ScenarioResults>& all_results) {
    ...
    std::cout << "• CPU sequential implementation (no parallelism)" << std::endl;
}
...
int main(int argc, char** argv) {
    std::cout << "╔════════════════════════════════════════════════════════════════════╗" << std::endl;
    std::cout << "║         CPU ORDERBOOK OPERATION TIMING BENCHMARK                   ║" << std::endl;
    std::cout << "╚════════════════════════════════════════════════════════════════════╝" << std::endl;
    ...
    if (argc > 1 && (std::string(argv[1]) == "-h" || std::string(argv[1]) == "--help")) {
        ...
        std::cout << "\nNote: This is CPU sequential implementation for comparison with GPU" << std::endl;
        return 0;
    }
    ...
    std::cout << "  Processing:        Sequential (CPU)" << std::endl;
    ...
    scenario1.name = "ADD Operations (CPU)";
    ...
    auto results1 = run_scenario(scenario1);
    ...
    scenario2.name = "LIMIT Order Insert+Match (CPU)";
    ...
    auto results2 = run_scenario(scenario2);
    ...
    scenario3.name = "CANCEL Operations (CPU)";
    ...
    auto results3 = run_scenario(scenario3);
    ...
    scenario4.name = "MARKET Order Insert+Match (CPU)";
    ...
    auto results4 = run_scenario(scenario4);
    ...
    std::cout << "\n✓ CPU Benchmark Complete!" << std::endl;
    std::cout << "\nCompare with GPU results using:" << std::endl;
    std::cout << "  ./benchmark_operations " << num_books << " " << msgs_per_book << std::endl;
